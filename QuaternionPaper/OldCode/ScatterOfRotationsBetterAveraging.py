	...
	for stepNo in range(len(odb.steps)):
		...
		#for f in range(1):
			...
			#for i in range(3):
				...
				averageQuaternion = averageQuat(GrainList)
				#will calculate scatter of quaternions from this average...
				#expression for dist between is 1 - <q1.q2>
				scatter= 0
				for q in GrainList:
					#print misorientation(averageQuaternion, q)
					scatter += misorientation(averageQuaternion, q)/len(GrainList)
				
					'''
					if (np.abs(dot(q,averageQuaternion)) > 0.7):
			...
			DataListSum = addUp(DataListList)/(1000 - anomalies)
			print( str(time) + '  ' + str(DataListSum) + '\n')
			
			outputText.append(str(time)+'  '+str(DataListSum) + '\n')
		
		temptime = time
	
	outFile_ev=open(mypathforoutput + "ScatterOverTimeUniaxialAdvancedAveragingTechnology2.txt",'w')
