...
def Normalise():# make a function to normalise those which are bit too big
	x = 0
def RootSumSq(array):
	summation = 0
...
def CalculateDeviationFromUnity(array):

	return abs(RootSumSq(array)-1)
	...
	for step in odb.steps.values():
		...
		for frame in step.frames:
			...
			for i in range(len(elemSets)):
				...
				outputText.append( str(CalculateDeviationFromUnity(averageQuaternion)))
			
	outFile_ev=open(mypathforoutput + str(frameNo) + "QuaternionInstability.txt",'w')
