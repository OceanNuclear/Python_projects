...
def function():
	...
	frameNoList = [60,120,180,240,300]
	...
	for frameNo in frameNoList:
		...
		for i in range(len(elemsets)):
			...
			for a in range (0,1):
				...
				for fieldNo in range(12): 
					...
					SDVFieldList.append(frame.fieldOutputs['SDV'+str(513+fieldNo)].getSubset(region = elemset))
		...
		outFile_ev = open(mypathforoutput + "DislocationDensityFrame" + str(frameNo+60) + "UnaxialNew.txt",'w')
