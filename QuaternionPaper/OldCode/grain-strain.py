...
def getMaxMises(odbName,elsetName):
	...
	for step in odb.steps.values():
		...
		for frame in step.frames:
            allFields = frame.fieldOutputs #just get all the fields
            
			if (allFields.has_key(Stress)): #go into all fields if it has stress field
                isStressPresent = 1
                stressSet = allFields[Stress] #get stress for for all fields in dis frame
